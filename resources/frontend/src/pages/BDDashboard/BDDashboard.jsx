    ...
    <Box sx={{ maxWidth: 1100, mx: 'auto', overflow: 'hidden' }}>
        ...
        >
          <CardContent sx={{ p: 2.5, '&:last-child': { pb: 2.5 }, display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            <Box sx={{ flex: 1, minWidth: 0 }}>
              ...
              <Typography variant="subtitle1" fontWeight={700} color="primary" sx={{ lineHeight: 1.3 }}>
                Tracking QR Code
              </Typography>
            </Box>
            <Button
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr 1fr', md: '1fr 1fr 1fr 1fr' }, gap: 2, mb: 3 }}>
          {kpiItems.map((kpi, idx) => (
            <Card
              ...
              sx={{
                ...(kpi.highlight
                  ? { backgroundColor: theme.palette.primary.main, color: '#fff', border: `2px solid ${theme.palette.primary.main}` }
                  : { border: `1px solid ${alpha(theme.palette.primary.main, 0.08)}` }
                ),
              }}
            >
              <CardContent sx={{ p: 2, '&:last-child': { pb: 2 } }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1 }}>
                  <Typography
                    ...
                    sx={{
                      ...
                      fontSize: '0.6rem',
                      color: kpi.highlight ? alpha('#fff', 0.7) : 'text.secondary',
                    }}
                  >
                    {kpi.label}
                  </Typography>
                  {!isMobile && (
                    <Box sx={{
                      ...
                      backgroundColor: kpi.highlight ? alpha('#fff', 0.15) : alpha(theme.palette.primary.main, 0.08),
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      '& svg': { fontSize: 18, color: kpi.highlight ? '#fff' : 'primary.main' },
                    }}>
                      {kpi.icon}
                    </Box>
                  )}
                ...
                <Typography
                  ...
                  sx={{
                    fontWeight: 700,
                    color: kpi.highlight ? '#fff' : (kpi.color === 'secondary' ? 'secondary.main' : 'text.primary'),
                    lineHeight: 1.2,
                  }}
                >
                  {kpi.value}
                ...
                {kpi.sub && (
                  <Typography
                    variant="caption"
                    sx={{
                      color: kpi.highlight ? alpha('#fff', 0.6) : (kpi.color === 'secondary' ? 'secondary.main' : 'text.secondary'),
                      fontSize: '0.65rem',
                    }}
                  >
                    {kpi.sub}
      ...
      {loading ? (
        ...
        <Paper sx={{ p: 4, textAlign: 'center' }}>
          <Typography color="text.secondary">No orders yet. Start sharing your QR code!</Typography>
        </Paper>
      ) : isMobile ? (
        /* Mobile: Card list */
        <Box>
